...
// ==========================================
// 🌟 修正版 FilterableDropdown (Fixed Portal 避免 Cutoff)
// ==========================================
const FilterableDropdown = ({
  ...
  const handleToggle = () => {
    ...
    if (!isOpen && selectRef.current) {
      ...
      const dropdownHeight = 260; // max-h-64
  ...
  return (
    <>
      <div
        ...
        className={`w-full h-[40px] px-3 py-2 border rounded-lg text-[13px] flex justify-between items-center transition-colors shadow-sm cursor-pointer ${
          disabled
      ...
      {isOpen && !disabled && (
        <>
          ...
          <div
            ref={dropdownMenuRef}
            style={dropdownStyle}
            className="fixed z-[9999] bg-white border border-slate-200 rounded-xl shadow-2xl flex flex-col max-h-64 overflow-hidden animate-in fade-in zoom-in-95 duration-100"
          >
            <div className="p-2 border-b border-slate-100 bg-slate-50 shrink-0">
              <div className="relative">
                ...
                <input
                  autoFocus
                  className="w-full border border-slate-200 bg-white rounded-lg pl-8 pr-3 py-1.5 text-[13px] font-bold focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-500/20 transition-all shadow-sm"
              ...
              {filteredOptions.length > 0 ? (
                filteredOptions.map((opt) => (
                  <div
                    ...
                    className={`px-3 py-2.5 text-[13px] rounded-lg cursor-pointer transition-colors font-bold ${
...
const QuotationCreatePage = () => {
  ...
  // 🌟 從全部料號中過濾出「包材相關」供外包裝/內包裝選擇，並加入清空選項
    ...
    <div className="w-full p-4 md:p-8 pb-16 max-w-7xl mx-auto bg-slate-50 min-h-screen font-sans text-slate-800 print:bg-white print:p-0">
      ...
      <form className="flex flex-col gap-8 w-full pb-6 print:hidden">
        ...
        <div className="mt-2 flex flex-col sm:flex-row justify-end gap-3.5 w-full">
          ...
          >
            <Save size={17} /> {isSubmitting ? "建立中..." : "建立報價單"}
          </button>
        </div>
      </form>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-slate-200 print:shadow-none print:w-full print:max-w-none print:max-h-none print:overflow-visible print:block">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50/80 shrink-0 print:hidden">
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={handlePrint}
                  className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-[13px] font-bold rounded-xl flex items-center gap-1.5 transition-all shadow-sm active:scale-95"
                >
